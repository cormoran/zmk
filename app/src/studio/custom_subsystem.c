...
static zmk_studio_Response list_custom_subsystems(const zmk_studio_Request *req) {
    LOG_DBG("");
    zmk_custom_ListCustomSubsystemRequest *request =
        &req->subsystem.custom.request_type.list_custom_subsystems;
...
static zmk_studio_Response call(const zmk_studio_Request *req) {
    ...
    const custom_subsystem_handler *handler = custom_subsys->handler;
    if (handler) {
